...
// Function to shift pixels and create variations
...
const createDataObject = async () => {
  const diceJsonPath = path.join(__dirname, 'data', 'dice.json');
  const inDice = require(diceJsonPath).data;
  const diceData = {};
  ...
  const outputPath = path.join(__dirname, 'data', 'dice_data.json');
